...
interface EditSectionModalProps {
  ...
  currentTitle: string;
  sectionId: string; // Nouvelle propriété pour identifier la section à modifier
}

const EditSectionModal = ({ onClose, onSubmit, currentTitle, sectionId }: EditSectionModalProps) => {
  const [title, setTitle] = useState(currentTitle);
  const [error, setError] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  // Mettre à jour le titre si currentTitle change
  useEffect(() => {
